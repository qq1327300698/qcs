...
$sql1 = "SELECT * FROM szzz_sp ORDER BY uptime DESC LIMIT 10";
        ...
        if($row["fenlei"]){
            if($xunh<5){
                ...
                <li class='tuijian_neirong'>
                            <a href='./szzz/szshiping.php?biaoti=".$row["bt"]."'' title='".$row["bt"]."'>
...
$sql1 = "SELECT * FROM xw_sp ORDER BY uptime DESC LIMIT 10";
$result1=mysqli_query($conn, $sql1);
$xunh=0;
date_default_timezone_set('Asia/Shanghai');
if(mysqli_num_rows($result1)>0){
    while ($row=mysqli_fetch_array($result1)) {
        if($row["fenlei"]){
            if($xunh<5){
                echo "
                <li class='tuijian_neirong'>
                            <a href='./xiangwang/szshiping.php?biaoti=".$row["bt"]."'' title='".$row["bt"]."'>
                            <img src='".$row["srcimg"]."' alt=''>
                            <div class='nr_xx'>
                                <p class='title'>".$row["bt"]."</p>
                                <div class='zhezhao'>
